...
export default function Sidebar({
  ...
  const navItems = [
    ...
    { id: "library", label: "Projects", icon: BookOpen },
    { id: "chapters", label: "Chapters", icon: FileSpreadsheet },
    { id: "composition", label: "Kanban", icon: Columns3 },
    { id: "assets", label: "Assets", icon: FolderOpen },
    { id: "review", label: "Review", icon: CheckSquare },
  ].filter((item) => !availableTabs || availableTabs.includes(item.id));
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all duration-200 hover:translate-x-1 ${isActive
